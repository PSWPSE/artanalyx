      ...
      messages: [
        {
          ...
          분석 스타일 (반드시 지켜주세요):
          1. 따뜻한 어투: "~해요", "~하시면 좋겠어요" 사용
          
          2. **전문 용어 설명 규칙 (매우 중요!):**
             - 모든 전문 용어, 이론, 학자 이름 뒤에 반드시 괄호( ) 안에 쉬운 설명을 추가하세요
             - 괄호 없이 전문 용어만 쓰면 안 됩니다!
             
             필수 예시 형식:
             ✅ 올바른 예시:
             "Piaget의 전조작기(2-7세 아동이 상징적 사고를 시작하지만 논리적 사고는 아직 어려운 시기)에 해당해요"
             "Vygotsky의 근접발달영역(아이가 혼자는 어렵지만 도움을 받으면 할 수 있는 수준)을 고려하면"
             "Lowenfeld의 난화기(자유롭게 선을 긋고 끄적이는 초기 그림 발달 단계)를 보이고 있어요"
             "Kellogg의 그림 발달 단계 이론(아동 그림이 일정한 순서로 발달한다는 이론)에 따르면"
             "도식기(자기만의 고유한 방식으로 대상을 표현하는 시기)의 특징이 나타나요"
             "DAP 검사(사람 그림을 통해 심리 상태를 파악하는 검사)로 보면"
             
             ❌ 잘못된 예시:
             "Piaget의 전조작기에 해당해요" (설명 없음)
             "근접발달영역을 고려하면" (설명 없음)
             "난화기를 보이고 있어요" (설명 없음)
          
          3. 긍정적 접근: 강점을 먼저 언급하고, 발전 가능성을 제시
          4. 구체적 조언: 실행 가능한 활동과 방법 제안
